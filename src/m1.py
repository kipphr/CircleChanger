...
class CircleChanger(object):
    ...
    def swallow(self, other_cc):
        ...
        new_c = rg.Point((self.circle.center.x+other_cc.circle.center.x)/2,(self.circle.center.y+other_cc.circle.center.y)/2)
        new_r = new_c.get_distance_from(self.circle.center)
        new_colors = self.colors + other_cc.colors
        return CircleChanger(new_c.x, new_c.y, new_r, 'red', new_colors)

    def change_color(self, index_of_color):
        """
        What comes in:
          -- self
          -- An non-negative integer that is less than the length
